...
## ☁️ Opção C: AWS EC2 (Deploy com Docker - Implementado)

Este foi o modelo de deploy implementado no projeto para a apresentação. O ecossistema completo de back-end está hospedado em um servidor **Amazon Linux 2023** rodando em uma instância **t3.micro**.

### 🔑 1. Comando de Conexão SSH (Executar no PowerShell Local)
Para conectar no servidor AWS a partir do seu computador pessoal:
```powershell
ssh -i "C:\Users\diego\OneDrive\Documentos\Faculdade\pizzaria-chave.pem" ec2-user@13.61.177.99
```

### 📦 2. Comandos executados no servidor AWS para o Deploy
Uma vez conectado via SSH, a pasta do projeto foi clonada e os containers foram inicializados:
```bash
...
# 3. Subir os microsserviços em segundo plano
docker-compose up -d
```

### 📊 3. Comandos para a Apresentação Acadêmica (Mostrar ao Professor)
Durante a apresentação com o professor, execute estes comandos dentro do terminal SSH da EC2 para demonstrar o deploy:

*   **Mostrar que todos os microsserviços estão ativos na nuvem:**
    ```bash
    docker ps
    ```
    *(Exibirá a tabela mostrando os containers da API REST, GraphQL API, Calculadora SOAP e Banco PostgreSQL todos online).*

*   **Acompanhar os logs das APIs e banco de dados em tempo real (Logs unificados):**
    ```bash
    # Na pasta do projeto (/home/ec2-user/Projeto-Pizzaria)
    docker-compose logs -f --tail=20
    ```
    *(Dica: Faça um pedido no frontend local ou envie um cálculo SOAP enquanto deixa essa tela aberta. Os logs de acesso piscarão no terminal na hora! Aperte `Ctrl + C` para sair).*

*   **Mostrar o consumo de recursos (RAM e Swap) da máquina virtual:**
    ```bash
    free -h
    ```

*   **Mostrar o armazenamento do disco EBS:**
    ```bash
    df -h
    ```
