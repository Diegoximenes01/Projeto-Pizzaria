...
export default function OrdersHistory() {
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto', paddingBottom: 48 }}>
      ...
      {pedidos.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px 24px', background: 'var(--card-bg)', borderRadius: 12, border: '1px solid var(--border)' }}>
          <div style={{ fontSize: 48, marginBottom: 16 }}>🍕</div>
          <h3 style={{ fontSize: 20, color: 'var(--text-main)', marginBottom: 12, fontWeight: 700 }}>
            Você nunca realizou um pedido antes!
          </h3>
          <p style={{ fontSize: 15, color: 'var(--text-muted)', marginBottom: 24, maxWidth: 450, margin: '0 auto 24px', lineHeight: '1.5' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          {pedidos.map((pedido) => {
            const numItens = pedido.itens.reduce((acc, curr) => acc + curr.quantidade, 0);
            return (
              <div 
                key={pedido.id} 
                style={{
              ...
              >
                {/* Header do Pedido */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border)', paddingBottom: 16, marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
                  <div>
                    <span style={{ fontSize: 14, color: 'var(--text-muted)', display: 'block', marginBottom: 4 }}>Código do pedido</span>
                ...
                <div style={{ marginBottom: 16 }}>
                  <span style={{ fontSize: 12, color: 'var(--text-muted)', display: 'block', marginBottom: 8, textTransform: 'uppercase', fontWeight: 600 }}>Itens ({numItens})</span>
                ...
                {/* Detalhes Adicionais */}
                <div style={{ background: 'var(--input-bg)', padding: 12, borderRadius: 8, fontSize: 13, color: 'var(--text-muted)', marginBottom: 16, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <div>
