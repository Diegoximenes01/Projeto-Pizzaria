...
export default function Auth() {
  ...
  useEffect(() => {
    if (location.state && typeof location.state.signup === 'boolean') {
      setMode(location.state.signup ? 'signup' : 'login');
    }
  }, [location.state]);
    ...
    try {
      if (mode === 'login') {
        const { data } = await client.query({
          query: LOGIN,
          variables: { cpf: cleanCpf, senha: formData.senha }
        });
        setUser(data.login);
        if (cart && cart.length > 0) {
          navigate('/entrega');
        ...
        const { data } = await cadastrar({ variables: { ...formData, cpf: cleanCpf } });
        ...
        await redefinirSenha({
          variables: { cpf: cleanCpf, novaSenha: formData.senha }
        });
